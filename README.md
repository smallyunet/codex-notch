...
- 鼠标移入实体刘海后展开周额度、重置时刻、倒计时和最近对话。
- 左侧使用高清 ChatGPT 标记，运行任务时显示更明显的脉冲光环；右侧可以显示“12 点起点顺时针圆环”或“波浪球”。
- 右键刘海或打开应用菜单中的“设置…”即可切换额度指示器；圆环和波浪球都支持把数字放在指标内或右侧。修改会立即生效并保存在本机。
- 周额度剩余不低于 20% 时圆环为绿色，低于 20% 时为红色；未返回周额度时显示灰色。
- 悬停展开当前活动任务、本周横向额度进度条、精确重置时刻和秒级倒计时；圆环只用于收起状态，点击任务直接打开 codex://threads/<thread-id>。
